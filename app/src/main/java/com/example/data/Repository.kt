...
class Repository(private val db: AppDatabase) {
    ...
    private suspend fun <T> safeDbCall(action: suspend () -> T): T? {
        return try {
            ...
            android.util.Log.e("Repository", "Database error: ${e.message}", e)
            null
        }
    }
    ...
    suspend fun updateBuddyConnection(buddyId: String, connected: Boolean) {
        safeDbCall { db.buddyDao().updateConnectionStatus(buddyId, connected) }
    }

    suspend fun insertBuddyMessage(message: BuddyMessage) {
        safeDbCall { db.buddyDao().insertBuddyMessage(message) }
    }
    ...
    suspend fun updateProfile(profile: UserProfile) {
        safeDbCall { db.userDao().insertOrUpdateProfile(profile) }
    }
    ...
    suspend fun addDiscussionPost(post: DiscussionPost) {
        safeDbCall { db.discussionDao().insertPost(post) }
    }

    suspend fun incrementPostLikes(postId: String) {
        safeDbCall { db.discussionDao().incrementLikes(postId) }
    }

    suspend fun insertChatMessage(message: MentorChat) {
        safeDbCall { db.chatDao().insertMessage(message) }
    }

    suspend fun clearAiChats() {
        safeDbCall { db.chatDao().clearChats(isAi = true) }
    }

    suspend fun updateLessonDownloaded(lessonId: String, downloaded: Boolean) {
        safeDbCall { db.lessonDao().updateDownloadedStatus(lessonId, downloaded) }
    }
    ...
    suspend fun updateUserLanguage(langCode: String) {
        safeDbCall { db.userDao().updateLanguage(langCode) }
    }

    suspend fun updateDataSaving(enabled: Boolean) {
        safeDbCall { db.userDao().updateDataSaving(enabled) }
    }

    suspend fun updateDownloadedOfflineStatus(enabled: Boolean) {
        safeDbCall { db.userDao().updateDownloadedOfflineStatus(enabled) }
    }

    suspend fun updatePremiumStatus(isPremium: Boolean) {
        safeDbCall { db.userDao().updatePremiumStatus(isPremium) }
    }

    suspend fun updatePlusStatus(isPlus: Boolean) {
        safeDbCall { db.userDao().updatePlusStatus(isPlus) }
    }
    ...
    suspend fun prepopulateDatabaseIfEmpty() {
        try {
            ...
            progresses.forEach { progress ->
                if (progress.isCompleted || progress.quizCompleted) {
                    unlockNextLesson(progress.lessonId)
                }
            }
        } catch (e: Exception) {
            android.util.Log.e("Repository", "Database seeding error: ${e.message}", e)
        }
    }
}
